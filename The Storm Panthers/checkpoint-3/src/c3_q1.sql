...
create temp table races_allegations as (
	...
	left join (
	    select data_area.id, count(distinct crid) as c
	    from data_allegation
	        ...
	        cross join data_area
	    where st_contains(data_area.polygon,data_allegation.point) = TRUE
	    ...
	    group by data_area.id, data_area.name
