...
order by ds.officer_id,year) y
    ...
    -- select count(case when allegation_count >= 1 and final_finding = 'SU' then 1 else null end) as officer_count,
           ...
           count(case when rank_changed = true and allegation_count >= 2 then 1 else null end)  as promotions,
           count(case when rank_changed = false and allegation_count >= 2 then 1 else null end) as not_promotions,
